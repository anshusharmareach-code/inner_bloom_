  ...
  const handleSubmit = async (e) => {
    ...
    if (db && userId) {
      ...
      try {
        ...
        const userRoot = ref(db, `users/${userId}/assessmentResults`);
        const newUserResultRef = push(userRoot);
        await set(newUserResultRef, payload);
        setHasSubmitted(true);
        console.log("GAD-7 result saved to public and user nodes ✅");
      } catch (error) {
